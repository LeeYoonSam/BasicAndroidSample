...
object TimeUtils {
    ...
    fun timestampToSnsFormat(timestamp: Long, timeStringProvider: TimeStringBaseProvider): String {
        ...
        return when {
            diffTime < SEC -> {
                timeStringProvider.getString(TimeStringBaseProvider.Code.JUST_NOW)
            }

            SEC.let { diffTime /= it; diffTime } < MIN -> {
                val snsFormatter = timeStringProvider.getString(TimeStringBaseProvider.Code.MIN)
                String.format(snsFormatter, diffTime)
            }

            MIN.let { diffTime /= it; diffTime } < HOUR -> {
                val snsFormatter = timeStringProvider.getString(TimeStringBaseProvider.Code.HOUR)
                String.format(snsFormatter, diffTime)
            }

            HOUR.let { diffTime /= it; diffTime } <= DAY -> {
                val snsFormatter = timeStringProvider.getString(TimeStringBaseProvider.Code.DAY)
                String.format(snsFormatter, diffTime)
            }
